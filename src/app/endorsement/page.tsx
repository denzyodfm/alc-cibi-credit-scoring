...
export default async function EndorsementPage() {
  ...
  const loans = allowed ? await prisma.loanApplication.findMany({ where: { status: "FOR_ENDORSEMENT", ...(canAccessAllBranches(user) ? {} : { branchId: user.branchId }) }, include: { applicantProfile: true, branch: true, loanOfficer: true, scorecard: true }, orderBy: { updatedAt: "asc" } }) : [];
