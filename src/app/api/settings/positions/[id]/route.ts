...
const roles=["SUPER_ADMIN","HEAD_OFFICE_ADMIN","HEAD_OFFICE_CREDIT_COMMITTEE","AREA_TEAM_LEADER","BRANCH_TEAM_LEADER","ACCOUNT_OFFICER","CASHIER","BOOKKEEPER","VIEWER"] as const;
...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){const actor=await requireUser();if(!canManageSetup(actor))return NextResponse.json({error:"Forbidden"},{status:403});const id=Number((await params).id);return NextResponse.json(await prisma.position.update({where:{id},data:schema.parse(await request.json())}));}
